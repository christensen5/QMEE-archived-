...
########## Thermodynamic models to fit to the data ##########

#~browser()
# Johnson-Lewin model
#~JoLe <- function(const, E, E_D, T_pk, temp)
#~{
#~    # Parameters
#~    # const  : Normalisation constant
#~    # E      : Activation energy 
#~    # E_D    : De-Activation energy
#~    # T_pk   : Temp at which trait reaches peak value
#~    
#~    return(const + log(exp(-E/(k * temp))/(1 + exp((1/k) * (((E_D/T_pk) + k * log(E/(E_D - E))) - (E_D/temp)))))) #Johnson-Lewin Model
#~}
...
OutNames <- c("ID", "Trait", "lnB0", "B0", "E", "E_D", "T_pk", "T_pkStart", "B_pk", "B_pkStart", "OriUnits", "StanUnits", "RSS", "R^2", "N_lessthanT_pk", "N_greaterthanT_pk","Habitat", "Latitude", "Longitude", "Species", "Citation", "FigureTable", "AcclimTemp", "AcclimTempDur", "AcclimTempDurUnits", "Input") #Headers for output
...
#  } 
  ...
  #~    E_bds <- c(0,2)
  #~    E_D_bds <- c(2,10)
  #~    T_pk_bds <- c(-50+273.15,50+273.15)
  ...
  #~    if(Results[i,"ID"] == 141) next
  
#~  plot(tmpData[,'ConTemp']- 273.15,tmpData[,'OriginalTraitValue'])
  
  NLSfit <- NULL #Initialize a null variable wihich will be replaced if tries succeed 
  for(j in 1:tries){
    #Schoolfield model (fitting using the Levenberg-Marqualdt algorithm)
    try(NLSfit <- nlsLM(log(OriginalTraitValue) ~ SchoolF(const,E,E_D,T_pk,ConTemp), tmpData,
  ...
  Results[i,"Trait"] <- as.character(tmpData$StandardisedTraitName[1])
